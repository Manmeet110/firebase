...
public class NewPostFragment extends Fragment {
    ...
    private final ActivityResultLauncher<Intent> grabadoraAudio =
        ...
        private void seleccionarImagen() {mediaTipo = "image";galeria.launch("image/*");}
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        publishButton.setOnClickListener(new View.OnClickListener() {
            ...
            @Override
            public void onClick(View view) {publicar();}
        });
    ...
    private void guardarEnFirestore(String postContent, String mediaUrl) {
        ...
        Post post = new Post(user.getUid(), user.getDisplayName(),
                (user.getPhotoUrl() != null ? user.getPhotoUrl().toString():"R.drawable.user"), postContent, mediaUrl, mediaTipo);
    ...
    private void pujaIguardarEnFirestore(final String postText) {
        FirebaseStorage.getInstance().getReference(mediaTipo + "/" + UUID.randomUUID()).putFile(mediaUri).continueWithTask(task ->
                task.getResult().getStorage().getDownloadUrl()).addOnSuccessListener(url ->
                guardarEnFirestore(postText, url.toString()));
    }

    private void guardarEnFirestore(String postContent) {
        FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();

        Post post = new Post(user.getUid(), user.getDisplayName(), (user.getPhotoUrl() != null ? user.getPhotoUrl().toString() : null),postContent, mediaUri.toString(), mediaTipo);
